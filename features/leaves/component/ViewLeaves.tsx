    ...
    return (
        ...
        >
            ...
            <Text style={[styles.sectionHeading, { color: muted }]}>
                {employees.length} EMPLOYEE{employees.length !== 1 ? "S" : ""} (Top 10 Entries)
            </Text>
...
function LeaveCard({
    ...
    const handleAction = (action: "approve" | "reject" | "cancel") => {
        const messages: Record<string, string> = {
            approve: `Approve ${employeeName}'s leave request?`,
            reject:  `Reject ${employeeName}'s leave request?`,
            cancel:  `Cancel ${employeeName}'s leave request?`,
        };
        Alert.alert(
            ...
            [
                { text: "No", style: "cancel" },
                {
                    ...
                    // TODO: wire up real mutation here, e.g. onApprove(r.id)
                    onPress: () => Alert.alert("Done", `Request ${action}d.`),
                },
            ]
        );
    };
