...
import useGetAbsentToday from "@/features/dashboard/hooks/useGetAbsentToday";
...
export default function AbsentToday() {
  ...
  const { absentEmployees, isLoading, refetch } = useGetAbsentToday();
  const [refreshing, setRefreshing] = React.useState(false);

  const data: PresentAbsentResponseT[] = absentEmployees ?? [];
  ...
  const accent = "#ef4444";
  const accentBg = isDark ? "rgba(239,68,68,0.12)" : "rgba(239,68,68,0.08)";
        ...
        <View style={styles.cardTopRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={[styles.badge, { backgroundColor: accentBg }]}>
              <Ionicons name="close-circle" size={12} color={accent} />
              <Text style={[styles.badgeText, { color: accent }]}>Absent Today</Text>
            </View>
          </View>
        </View>
  ...
  return (
    ...
    <View style={[styles.container, { backgroundColor: bg }]}>
      <Stack.Screen options={{ title: 'Absent Today', headerLeft: () => (<TouchableOpacity onPress={() => router.push("/")} style={{ paddingHorizontal: 10 }}> <Ionicons name="arrow-back" size={24} color="green" /> </TouchableOpacity>), }} />
      <View style={[styles.headerCard, { backgroundColor: card, borderColor: border }]}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.title, { color: fg }]}>Absent Today</Text>
          <Text style={[styles.subtitle, { color: muted }]}>
            {data.length} employee{data.length === 1 ? "" : "s"} are absent
          </Text>
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={[styles.emptyCard, { backgroundColor: card, borderColor: border }]}>
              ...
              <Text style={[styles.emptyText, { color: fg }]}>No absent employees found.</Text>
              <Text style={[styles.emptySubText, { color: muted }]}>
                There is no absence data to display right now.
              </Text>
            </View>
          }
        />
...
const styles = StyleSheet.create({
   safe: { flex: 1,marginTop:0, paddingTop:0 },
  container: {
    flex: 1,
